...
class CreatDatasetStage2_5clf(Dataset):
    def __init__(self, name, label, data_rootdir, fold, traintestflag='train'):
        self.allname = name
        self.alllabel = label
        self.data_rootdir = data_rootdir
        self.traintestflag = traintestflag
        age_gender_csv = pd.read_csv("/home/yxpt/Desktop/Projects/H_PDN_new_further20200125/Files/AgeGender_combinedata.csv")
    ...
    def __getitem__(self, item):
        ...
        if self.traintestflag == 'train':
            ...
            # for pair in ['BDHC', 'BDMDD', 'BDSZ',
            #              'BDOCD', 'MDDHC', 'MDDSZ', 'MDDOCD', 'SZHC', 'SZOCD', 'OCDHC']:
            for pair in ['BDOthers', 'MDDOthers', 'SZOthers', 'OCDOthers', 'HCOthers',
                         'BDHC', 'BDMDD', 'BDSZ',
                         'BDOCD', 'MDDHC', 'MDDSZ', 'MDDOCD', 'SZHC', 'SZOCD', 'OCDHC']:
            # for pair in ['BDOthers', 'MDDOthers', 'SZOthers', 'OCDOthers', 'HCOthers']:
            # for pair in ['MDDOthers', 'SZOthers', 'OCDOthers', 'HCOthers',
            #              'BDHC', 'BDMDD', 'BDOCD', 'MDDSZ', 'MDDOCD', 'SZHC', 'SZOCD']:
                feature_name = 'train_5clffeature_%s_fold%d.nii.npy' % (pair, self.fold)
            ...
            # for pair in ['MDDOthers', 'SZOthers', 'OCDOthers', 'HCOthers',
            #              'BDHC', 'BDMDD', 'BDOCD', 'MDDSZ', 'MDDOCD', 'SZHC', 'SZOCD']:
                feature_name = 'val_5clffeature_%s_fold%d.nii.npy' % (pair, self.fold)
                feature_name_list.append(feature_name)

        casename = allname[item]
        current_feature_list = []
        for feature_name in feature_name_list:
            feature_pth = os.path.join(self.data_rootdir, casename, feature_name)
            feature_aslist = np.load(feature_pth).tolist()
            current_feature_list = current_feature_list + feature_aslist
        case_feature_np = np.array(current_feature_list)
